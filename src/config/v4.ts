...
function flattenIssues(error: v.ValiError<typeof configSchema>, parentPath = ""): ConfigIssue[] {
  const result: ConfigIssue[] = [];

  for (const issue of error.issues) {
    const currentPath = [parentPath, ...(issue.path ?? []).map((item) => String(item.key))]
      .filter(Boolean)
      .join(".");

    if (issue.issues) {
      result.push(...flattenIssuesFromIssue(issue, currentPath));
    } else {
      result.push({
        path: currentPath,
        expected: issue.expected,
...
function flattenIssuesFromIssue(issue: v.BaseIssue<unknown>, parentPath: string): ConfigIssue[] {
  const currentPath = [parentPath, ...(issue.path ?? []).map((item) => String(item.key))]
    .filter(Boolean)
    .join(".");

  if (issue.issues) {
    return issue.issues.flatMap((nestedIssue) => flattenIssuesFromIssue(nestedIssue, currentPath));
  }
...
function mergeUnionIssues(issues: ConfigIssue[]): ConfigIssue {
  ...
  const matched = issues.filter(
    (item) => item.path === issue.path && item.received === issue.received,
  );
  ...
  return {
    ...
    expected: matched
      ...
      .filter((expected): expected is string => expected !== null)
      .join(" | "),
  };
}
