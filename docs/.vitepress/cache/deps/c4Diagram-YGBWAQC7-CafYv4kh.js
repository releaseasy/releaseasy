...
import { H as setAccDescription, U as setAccTitle, c as configureSvgSize, r as assignWithDepth_default, s as common_default, v as getAccDescription, x as getConfig2, y as getAccTitle, z as sanitizeText } from "./chunk-O7XYJQB3-mKDgYvQm.js";
import { _ as wrapLabel, n as calculateTextHeight, r as calculateTextWidth } from "./chunk-ZIGJFQKS-CX9G_8OE.js";
import "./chunk-7PRAP22T-BsZhkbcM.js";
import "./chunk-742MDFTN-DsvLQSVM.js";
import "./chunk-MBY4JIJT-DQOOXU90.js";
import { r as intersect_default, s as shapes } from "./chunk-7INBJB4K-Cqr6ftBk.js";
import { a as drawRect } from "./chunk-GWA4HPMP-BXj13iAN.js";
//#region node_modules/.pnpm/mermaid@12.0.0/node_modules/mermaid/dist/chunks/mermaid.core/c4Diagram-YGBWAQC7.mjs
var parser = (function() {
...
var getRequiredConfig = /* @__PURE__ */ __name((section) => getConfig2()[section], "getRequiredConfig");
...
var addPersonOrSystem = /* @__PURE__ */ __name(function(typeC4Shape, alias, label, descr, sprite, tags, link) {
	...
	assignAttributes(personOrSystem, {
		sprite,
		tags,
		link
	});
...
var drawRels = /* @__PURE__ */ __name((elem, rels2, conf2, diagramId) => {
	...
	for (const rel of rels2) {
		...
		_drawTextCandidateFunc(conf2)(rel.label.text, relsElem, Math.min(rel.startPoint.x, rel.endPoint.x) + Math.abs(rel.endPoint.x - rel.startPoint.x) / 2 + offsetX, Math.min(rel.startPoint.y, rel.endPoint.y) + Math.abs(rel.endPoint.y - rel.startPoint.y) / 2 + offsetY, labelWidth, rel.label.height, { fill: textColor }, messageConf);
		if (rel.techn && rel.techn.text !== "") {
			messageConf = conf2.messageFont();
			_drawTextCandidateFunc(conf2)("[" + rel.techn.text + "]", relsElem, Math.min(rel.startPoint.x, rel.endPoint.x) + Math.abs(rel.endPoint.x - rel.startPoint.x) / 2 + offsetX, Math.min(rel.startPoint.y, rel.endPoint.y) + Math.abs(rel.endPoint.y - rel.startPoint.y) / 2 + conf2.messageFontSize + 5 + offsetY, Math.max(labelWidth, rel.techn.width), rel.techn.height, {
				fill: textColor,
				"font-style": "italic"
			}, messageConf);
		}
...
var getDiagramRoot = /* @__PURE__ */ __name((id, securityLevel) => {
	if (securityLevel === "sandbox") {
		const doc = select_default("#i" + id).node()?.contentDocument;
		if (!doc) throw new Error(`Sandbox iframe #i${id} is missing its content document`);
		return {
...
var resolveNodeShape = /* @__PURE__ */ __name((shape) => {
	...
	if (shape.tags) for (const tag of shape.tags.split(",")) {
		const tagged = keywordShape(tag.trim());
		if (tagged) return tagged;
	}
	const typeC4Shape = shape.typeC4Shape.text;
	if (typeC4Shape === "person" || typeC4Shape === "external_person") return "person";
	if (DB_SHAPES.has(typeC4Shape)) return "cylinder";
	if (QUEUE_SHAPES.has(typeC4Shape)) return "h-cyl";
	return "rounded";
}, "resolveNodeShape");
...
var buildC4Node = /* @__PURE__ */ __name((shape, config, padding, look, elementWidth) => {
	...
	if (isExternal(typeC4Shape)) cssClasses.push("c4-external");
	const nodeShape = resolveNodeShape(shape);
	const cssStyles = elementCssStyles(shape, config);
	if (nodeShape === "rounded" || nodeShape === "fr-rect") cssStyles.push("rx:12px", "ry:12px");
	return {
		id: shape.alias,
...
function calcC4ShapeTextWH(textType, c4Shape, c4ShapeTextWrap, textConf, textLimitWidth) {
	...
	if (!textElement.width) {
		if (c4ShapeTextWrap) {
			textElement.text = wrapLabel(textElement.text, textLimitWidth, textConf);
			...
			for (const line of lines) {
				textElement.width = Math.max(calculateTextWidth(line, textConf), textElement.width);
...
var drawC4ShapeArray = /* @__PURE__ */ __name(async function(currentBounds, diagram2, c4ShapeArray2, c4ShapeKeys) {
	...
	const shapeHandlerFor = /* @__PURE__ */ __name((node) => {
		...
		if (!handler) throw new Error(`C4: no shape handler for "${node.shape}"`);
		return handler;
	}, "shapeHandlerFor");
	await Promise.all(c4Shapes.map(async (c4Shape) => {
	...
	for (const c4Shape of c4Shapes) currentBounds.insert(c4Shape);
	await Promise.all(c4Shapes.map(async (c4Shape) => {
		const node = buildC4Node(c4Shape, conf, conf.c4ShapePadding, look, conf.width);
		node.domId = `${diagramId}-${node.id}`;
		node.x = c4Shape.x + c4Shape.width / 2;
		node.y = c4Shape.y + c4Shape.height / 2;
		const positioned = diagram2.append("g").attr("transform", `translate(${c4Shape.x + c4Shape.width / 2}, ${c4Shape.y + c4Shape.height / 2})`);
...
var getIntersectPoint = /* @__PURE__ */ __name(function(fromNode, endPoint) {
	if (!fromNode.intersect) throw new Error(`C4 element "${fromNode.alias}" has no intersect function. Please report this to https://github.com/mermaid-js/mermaid/issues`);
	const { x, y } = fromNode.intersect(endPoint);
	return new Point(x, y);
}, "getIntersectPoint");
var getIntersectPoints = /* @__PURE__ */ __name(function(fromNode, endNode) {
	const endIntersectPoint = {
		x: 0,
		y: 0
	};
...
var drawRels2 = /* @__PURE__ */ __name(function(diagram2, rels2, getC4ShapeObj, diagObj, diagramId) {
	...
	for (const rel of rels2) {
		...
		if (!fromNode || !endNode) throw new Error(`C4 rel "${rel.from}" -> "${rel.to}" references an unknown shape or boundary`);
		const points = getIntersectPoints(fromNode, endNode);
		if (!points.startPoint || !points.endPoint) throw new Error(`Could not calculate intersection points for rel "${rel.from}" -> "${rel.to}"`);
...
async function drawInsideBoundary(diagram2, parentBoundaryAlias, parentBounds, currentBoundaries, diagObj) {
	...
	for (const [i, currentBoundary] of currentBoundaries.entries()) {
		...
		const label = calcC4ShapeTextWH("label", currentBoundary, currentBoundaryTextWrap, currentBoundaryLabelConf, currentBounds.data.widthLimit);
		label.Y = Y + 8;
		Y = label.Y + label.height;
		if (currentBoundary.type && currentBoundary.type.text !== "") {
			currentBoundary.type.text = "[" + currentBoundary.type.text + "]";
			const type = calcC4ShapeTextWH("type", currentBoundary, currentBoundaryTextWrap, boundaryFont(conf), currentBounds.data.widthLimit);
			type.Y = Y + 5;
		...
		if (currentBoundary.descr && currentBoundary.descr.text !== "") {
			...
			const descr = calcC4ShapeTextWH("descr", currentBoundary, currentBoundaryTextWrap, currentBoundaryDescrConf, currentBounds.data.widthLimit);
			descr.Y = Y + 20;
		...
		if (currentPersonOrSystemKeys.length > 0) await drawC4ShapeArray(currentBounds, diagram2, currentPersonOrSystemArray, currentPersonOrSystemKeys);
		parentBoundaryAlias = currentBoundary.alias;
		const nextCurrentBoundaries = db.getBoundaries(parentBoundaryAlias);
		if (nextCurrentBoundaries.length > 0) await drawInsideBoundary(diagram2, parentBoundaryAlias, currentBounds, nextCurrentBoundaries, diagObj);
...
var c4Renderer_default = {
	...
	setConf,
	draw: /* @__PURE__ */ __name(async function(_text, id, _version, diagObj) {
		...
		await drawInsideBoundary(diagram2, "", screenBounds, db.getBoundaries(""), diagObj);
		...
		const height = globalBoundaryMaxY - boxStarty + 2 * conf.diagramMarginY;
		const boxWidth = globalBoundaryMaxX - boxStartx;
		const width = boxWidth + 2 * conf.diagramMarginX;
		if (title2) diagram2.append("text").text(title2).attr("x", boxWidth / 2 - 4 * conf.diagramMarginX).attr("y", boxStarty + conf.diagramMarginY);
		configureSvgSize(diagram2, height, width, conf.useMaxWidth);
		const extraVertForTitle = title2 ? 60 : 0;
		diagram2.attr("viewBox", boxStartx - conf.diagramMarginX + " -" + (conf.diagramMarginY + extraVertForTitle) + " " + width + " " + (height + extraVertForTitle));
...
var elementFontStyles = /* @__PURE__ */ __name(() => {
	...
	for (const type of C4_ELEMENT_TYPES) {
		...
		if (fontFamily) rule.style.setProperty("font-family", fontFamily);
		if (fontSize) rule.style.setProperty("font-size", typeof fontSize === "number" ? `${fontSize}px` : fontSize);
		if (fontWeight) rule.style.setProperty("font-weight", String(fontWeight));
	}
