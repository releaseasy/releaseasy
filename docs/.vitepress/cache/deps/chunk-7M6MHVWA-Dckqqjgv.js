...
//#region node_modules/.pnpm/mermaid@12.0.0/node_modules/mermaid/dist/chunks/mermaid.core/chunk-7M6MHVWA.mjs
...
You have to call mermaid.initialize.`);
	...
	*/
	...
	getData() {
		...
		for (const sg of subGraphs) for (const childId of sg.nodes) if (this.subGraphLookup.has(childId)) subGraphParent.set(childId, sg.id);
		const declarationIndex = /* @__PURE__ */ new Map();
		const childrenOf = /* @__PURE__ */ new Map();
		for (const sg of subGraphs) {
			const parent = subGraphParent.get(sg.id);
			if (parent !== void 0) childrenOf.set(parent, [...childrenOf.get(parent) ?? [], sg.id]);
		}
		let nextDeclarationIndex = 0;
		const walk = /* @__PURE__ */ __name((sgId) => {
			declarationIndex.set(sgId, nextDeclarationIndex++);
			for (const childId of childrenOf.get(sgId) ?? []) walk(childId);
		}, "walk");
		...
		for (let i = subGraphs.length - 1; i >= 0; i--) {
			const subGraph = subGraphs[i];
			if (hiddenIds.has(subGraph.id)) continue;
			if (subGraph.metadata?.view === "collapsed") nodes.push({
				id: subGraph.id,
		...
		e.forEach((rawEdge, index) => {
			...
			if (start === end && (collapsedAncestorMap.has(rawEdge.start) || collapsedAncestorMap.has(rawEdge.end))) return;
...
var genColor = /* @__PURE__ */ __name((options) => {
	...
	for (let i = 0; i < colorSlotCount(options.THEME_COLOR_LIMIT, borderColorArray); i++) {
		...
		const collapsedRule = /* @__PURE__ */ __name((suffix) => `${slot}.node ${suffix}, ${slot}.rough-node ${suffix}`, "collapsedRule");
		const laneRule = /* @__PURE__ */ __name((suffix) => `${slot}.swimlane.cluster .swimlane-title${suffix}, ${slot}.swimlane.cluster .swimlane-body${suffix}`, "laneRule");
		sections += `
...
var styles_default = /* @__PURE__ */ __name((options) => `${genColor(options)}
  .label {
  ...
  ${getIconStyles()}
`, "getStyles");
var createFlowDiagram = /* @__PURE__ */ __name(({ styles = styles_default } = {}) => ({
