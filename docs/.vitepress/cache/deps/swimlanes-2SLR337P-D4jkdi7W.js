...
import { r as createCommonLayoutRenderer } from "./chunk-3FUC2YCW-s_D-ng71.js";
import { t as applyLineJumpsToSvg } from "./chunk-NETBCI7D-TmCpuKT1.js";
//#region node_modules/.pnpm/mermaid@12.0.0/node_modules/mermaid/dist/chunks/mermaid.core/swimlanes-2SLR337P.mjs
function applySwimlaneLineJumps(data4Layout, { measure }) {
	const lineHopsConfig = data4Layout.config?.swimlane?.lineHops;
	if (lineHopsConfig === false) return;
...
function removeCycles_DFS(g) {
	...
	const seeds = [...gn.nodes.filter((v) => (inDegree.get(v) ?? 0) === 0), ...gn.nodes.filter((v) => (inDegree.get(v) ?? 0) !== 0)];
	for (const v of seeds) if (color[v] === 0) dfs(v);
