...
import { o as markerOffsets } from "./chunk-Z7XXMR3K-Dp6XDiuE.js";
//#region node_modules/.pnpm/mermaid@12.0.0/node_modules/mermaid/dist/chunks/mermaid.core/chunk-NETBCI7D.mjs
var ROUNDED_CORNER_RADIUS = 5;
var CORNER_EPSILON = 1e-5;
var CORNER_JUMP_CLEARANCE = 2;
var MIN_USEFUL_RADIUS_RATIO = .6;
...
function crossingSitsInRoundedCorner(edge, segIndex, t) {
	if (edge.curve !== "rounded") return false;
	...
	if (entering && d < entering.cutLen) return true;
	...
	for (let i = 0; i < edges.length; i++) {
		...
		for (let j = i + 1; j < edges.length; j++) {
			...
			for (const [si, segA] of segmentsA.entries()) for (const [sj, segB] of segmentsB.entries()) {
				...
				if (!hit) continue;
				if (crossingSitsInRoundedCorner(edgeA, si, hit.tA) || crossingSitsInRoundedCorner(edgeB, sj, hit.tB)) continue;
...
function rewriteEdgePath(edge, jumps, config) {
	...
	for (let i = 0; i < segments.length; i++) {
		...
		for (const j of segJumps) {
			if (j.r < minUsefulRadius) continue;
...
function applyLineJumpsToSvg(edgePathsGroup, edges, config) {
	...
	for (const el of groupNode.querySelectorAll("path[data-id]")) {
		...
		if (id !== null && !pathByDataId.has(id)) pathByDataId.set(id, el);
	}
	const renderedEdges = [];
...
//#endregion
export { applyLineJumpsToSvg as t };
