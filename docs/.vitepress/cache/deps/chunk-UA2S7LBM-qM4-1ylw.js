...
import { T as getEffectiveHtmlLabels, g as evaluate, x as getConfig2 } from "./chunk-O7XYJQB3-mKDgYvQm.js";
import { a as userNodeOverrides, i as styles2String, t as compileStyles } from "./chunk-7PRAP22T-BsZhkbcM.js";
import { n as createText } from "./chunk-MBY4JIJT-DQOOXU90.js";
import { s as stampColorSlot } from "./chunk-J5ZVWO5B-DBdtyzdO.js";
import { t as at } from "./rough.esm-CukKpxpo.js";
import { i as intersect_rect_default, n as createRoundedRectPathD, t as createLabel_default } from "./chunk-7INBJB4K-Cqr6ftBk.js";
import { n as getSubGraphTitleMargins } from "./chunk-5DYCD2WN-CR4S9APN.js";
//#region node_modules/.pnpm/mermaid@12.0.0/node_modules/mermaid/dist/chunks/mermaid.core/chunk-UA2S7LBM.mjs
...
var rect = /* @__PURE__ */ __name(async (parent, node) => {
	...
	let text;
	if (node.labelType === "markdown") text = await createText(labelEl, node.label, {
		style: node.labelStyle,
		useHtmlLabels,
		isNode: true,
		width: node.width
	});
	else text = await createLabel_default(labelEl, node.label, node.labelStyle || "", false, true);
	let bbox = text.getBBox();
...
var roundedWithTitle = /* @__PURE__ */ __name(async (parent, node) => {
	const siteConfig = getConfig2();
	const { theme, themeVariables, handDrawnSeed } = siteConfig;
	const { altBackground, borderColorArray, compositeBackground, compositeTitleBackground, nodeBorder } = themeVariables;
...
var divider = /* @__PURE__ */ __name((parent, node) => {
	const { theme, themeVariables, handDrawnSeed } = getConfig2();
...
var createContainerGroup = /* @__PURE__ */ __name(async (parent, node, opts) => {
	...
	if (node.look === "handDrawn") {
		...
		if (opts.strokeDash) rectEl.attr("stroke-dasharray", opts.strokeDash.join(", "));
	}
	if (hasLabel) {
		const { subGraphTitleTopMargin } = getSubGraphTitleMargins(siteConfig);
		labelEl.attr("transform", `translate(${node.x - bbox.width / 2}, ${node.y - node.height / 2 + subGraphTitleTopMargin})`);
...
var flowGroup = /* @__PURE__ */ __name(async (parent, node) => {
...
var getUsecaseSystemBoundaryGeometry = /* @__PURE__ */ __name((node, labelBBox) => {
...
var shapes = {
	...
	kanbanSection,
	flowGroup,
	usecaseSystemBoundary: /* @__PURE__ */ __name(async (parent, node) => {
		...
		const boundaryType = node.boundaryType || "rect";
		const shapeSvg = parent.insert("g").attr("class", `cluster usecase-system-boundary usecase-system-boundary-${boundaryType} ${node.cssClasses}`).attr("id", typeof node.domId === "string" ? node.domId : node.id).attr("data-boundary-type", boundaryType).attr("data-look", node.look);
		stampColorSlot(shapeSvg, node.colorIndex, theme, borderColorArray);
		const useHtmlLabels = getEffectiveHtmlLabels(siteConfig);
		...
		const { width, height, x, y, bodyY, bodyHeight, tabHeight, tabWidth } = getUsecaseSystemBoundaryGeometry(node, bbox);
		...
		if (node.look === "handDrawn") {
			...
			if (boundaryType === "package") shapeSvg.insert("rect", ":first-child").attr("class", "boundary-tab system-boundary-package-tab label-container").attr("style", nodeStyles).attr("x", x).attr("y", y).attr("width", tabWidth).attr("height", tabHeight);
		}
		const { subGraphTitleTopMargin } = getSubGraphTitleMargins(siteConfig);
		if (boundaryType === "package") labelEl.attr("transform", `translate(${x + (tabWidth - bbox.width) / 2}, ${y + (tabHeight - bbox.height) / 2})`);
		else labelEl.attr("transform", `translate(${node.x - bbox.width / 2}, ${y + subGraphTitleTopMargin})`);
		if (labelStyles) {
...
var clusterPaintsTitle = /* @__PURE__ */ __name((shape) => !UNTITLED_CLUSTER_SHAPES.has(shape ?? "rect"), "clusterPaintsTitle");
...
//#endregion
export { clusterPaintsTitle as n, insertCluster as r, clear as t };
