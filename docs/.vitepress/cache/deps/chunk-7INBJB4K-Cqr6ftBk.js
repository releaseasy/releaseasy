...
import { A as hasKatex, B as sanitizeText3, M as parseGenericTypes, T as getEffectiveHtmlLabels, b as getConfig, f as defaultConfig_default, g as evaluate, x as getConfig2, z as sanitizeText } from "./chunk-O7XYJQB3-mKDgYvQm.js";
import { a as decodeEntities, p as parseFontSize, r as calculateTextWidth, u as handleUndefinedAttr } from "./chunk-ZIGJFQKS-CX9G_8OE.js";
import { a as userNodeOverrides, i as styles2String, r as solidStateFill, t as compileStyles } from "./chunk-7PRAP22T-BsZhkbcM.js";
import { n as isIconAvailable, t as getIconSVG } from "./chunk-742MDFTN-DsvLQSVM.js";
import { n as createText, r as fastdom_default2 } from "./chunk-MBY4JIJT-DQOOXU90.js";
import { s as stampColorSlot } from "./chunk-J5ZVWO5B-DBdtyzdO.js";
import { t as at } from "./rough.esm-CukKpxpo.js";
//#region node_modules/.pnpm/mermaid@12.0.0/node_modules/mermaid/dist/chunks/mermaid.core/chunk-7INBJB4K.mjs
var ELEMENT_NODE = 1;
var TEXT_NODE = 3;
...
var c4LabelHelper = /* @__PURE__ */ __name(async (parent, node, classes) => {
	...
	const sections = [
		{
			text: typeof node.label === "string" ? node.label : node.label?.[0] ?? "",
			cssClass: "c4-name"
		},
		{
	...
	const width = config.c4?.wrap ?? true ? wrapWidth : Number.POSITIVE_INFINITY;
	const rendered = await Promise.all(sections.map(async (section) => {
		const sectionEl = labelEl.append("g").attr("class", section.cssClass);
		const textEl = await createText(sectionEl, sanitizeText(decodeEntities(section.text ?? ""), config), {
			useHtmlLabels: false,
			markdown: false,
			isNode: true,
			width,
	...
	return {
		shapeSvg,
		bbox: labelEl.node().getBBox(),
		halfPadding: (node.padding ?? 0) / 2,
		label: labelEl
	};
}, "c4LabelHelper");
var withMinWidth = /* @__PURE__ */ __name((bbox, minWidth) => {
	const { x, y, width, height } = bbox;
	if (width >= minWidth) return bbox;
	if (!("left" in bbox)) return {
		x: x - (minWidth - width) / 2,
		y,
		width: minWidth,
		height
	};
...
var labelHelper = /* @__PURE__ */ __name(async (parent, node, _classes) => {
	if (node.stereotype !== void 0) return c4LabelHelper(parent, node, _classes);
	let cssClasses;
	...
	if (useHtmlLabels) {
		...
		bbox = await fastdom_default2.measure(() => text2.getBBox());
		bbox = withMinWidth(bbox, minLabelWidth);
	}
...
var insertLabel = /* @__PURE__ */ __name(async (parent, label, options) => {
	...
	if (getEffectiveHtmlLabels(getConfig2())) {
		...
		dv.attr("width", bbox.width);
		dv.attr("height", bbox.height);
	} else bbox = await fastdom_default2.measure(() => text2.getBBox());
	if (useHtmlLabels) labelEl.attr("transform", "translate(" + -bbox.width / 2 + ", " + -bbox.height / 2 + ")");
	else labelEl.attr("transform", "translate(0, " + -bbox.height / 2 + ")");
...
async function bucket(parent, node, { config: { themeVariables } }) {
	...
	const rimRy = Math.max(Math.min(w * .08, 12), 5);
	...
	if (node.look === "handDrawn") {
		const roughNode = at.svg(shapeSvg).path(body, userNodeOverrides(node, {}));
	...
	label.attr("transform", `translate(${-(bbox.width / 2) - (bbox.x - (bbox.left ?? 0))}, ${bodyCenterY - bbox.height / 2 - (bbox.y - (bbox.top ?? 0))})`);
	const ARC_SEGMENTS = 12;
	const arc = /* @__PURE__ */ __name((rx, cy, ySign) => Array.from({ length: 13 }, (_, i) => {
		const theta = Math.PI - i * Math.PI / ARC_SEGMENTS;
		return {
			x: rx * Math.cos(theta),
			y: cy + ySign * rimRy * Math.sin(theta)
		};
	}), "arc");
...
async function collapsedGroup(parent, node) {
	const style = getCollapsedStyle(node.metadata?.containerType);
	...
	const totalHeight = Math.max(titleHeight + SEPARATOR_GAP + INDICATOR_ROW_HEIGHT + padding * 2, node?.height ?? 0);
	const x = -totalWidth / 2;
	const y = -totalHeight / 2;
	const labelShiftY = -(SEPARATOR_GAP + INDICATOR_ROW_HEIGHT) / 2;
	const labelEl = shapeSvg.select(".label");
	if (labelEl) {
		if (node.useHtmlLabels ?? getEffectiveHtmlLabels(getConfig2())) labelEl.attr("transform", `translate(${-bbox.width / 2}, ${-bbox.height / 2 + labelShiftY})`);
		else labelEl.attr("transform", `translate(0, ${-bbox.height / 2 + labelShiftY})`);
	}
	let rect;
	if (node.look === "handDrawn") {
		const rc = at.svg(shapeSvg);
		...
		const roughNode = rc.path(createRoundedRectPathD(x, y, totalWidth, totalHeight, style.rx), roughOpts);
		...
		if (style.strokeWidth !== void 0) rect.attr("stroke-width", style.strokeWidth + "px");
	}
	...
	const dotRadius = 2.5;
	const dotSpacing = 10;
	for (let i = -1; i <= 1; i++) shapeSvg.append("circle").attr("class", "collapsed-indicator").attr("cx", i * dotSpacing).attr("cy", dotY).attr("r", dotRadius).attr("fill", stroke);
...
var getArrowPoints = /* @__PURE__ */ __name((duplicatedDirections, bbox, node, totalWidth) => {
	...
	return (arrowPointFactories[getDirectionKey(directions)] ?? arrowPointFactories[POINT_KEY])({
		height,
		midpoint,
		padding,
		width
...
async function block_arrow(parent, node) {
	...
	const width = blockNode.positioned && (blockNode.widthInColumns ?? 1) > 1 && nodeWidth > naturalWidth ? nodeWidth : naturalWidth;
...
async function browser(parent, node, { config: { themeVariables } }) {
	...
	const group = shapeSvg.insert("g", ":first-child").attr("class", "basic label-container");
	if (node.look === "handDrawn") {
		const roughNode = at.svg(shapeSvg).path(createRoundedRectPathD(-w / 2, top, w, h, radius), userNodeOverrides(node, {}));
		group.node()?.appendChild(roughNode);
		if (cssStyles) group.attr("style", cssStyles);
	...
	for (let i = 0; i < 3; i++) group.append("circle").attr("cx", -w / 2 + 12 + i * 9).attr("cy", top + barHeight / 2).attr("r", 2.5).attr("style", `fill:${accent};stroke:none`);
	group.append("rect").attr("class", "browser-address-bar").attr("x", -w / 2 + 44).attr("y", top + 4).attr("width", Math.max(w - 56, 10)).attr("height", 10).attr("rx", 3).attr("ry", 3).attr("style", `fill:none;stroke:${accent};stroke-width:1px;opacity:0.6`);
	updateNodeBounds(node, group);
...
async function circle(parent, node, options) {
	...
	const radius = node.look === "neo" ? labelRadius + 32 : labelRadius + padding;
	let circleElem;
	const { cssStyles } = node;
	if (node.look === "handDrawn") {
		const rc = at.svg(shapeSvg);
...
async function composite(parent, node) {
	...
	const { shapeSvg, bbox, halfPadding } = await labelHelper(parent, compositeNode, [
		"node",
		compositeNode.cssClasses,
		compositeNode.class
	].filter(Boolean).join(" "));
...
async function curvedTrapezoid(parent, node) {
	...
	const w = Math.max(minWidth, bbox.width + labelPaddingX * 2 + sideClearance * 2, (bbox.width + labelPaddingX * 2) * 1.25, node?.width ?? 0);
	const { cssStyles } = node;
	const rc = at.svg(shapeSvg);
	const options = userNodeOverrides(node, {});
	if (node.look !== "handDrawn") {
...
async function person(parent, node) {
	...
	const headRadius = Math.min(Math.max(w * .23, 16), 56);
	const overlap = headRadius * .27;
	const bodyHeight = Math.max(bbox.height + padding * 2, node.height ? node.height - (2 * headRadius - overlap) : 0);
	const bodyRadius = Math.min(w * .177, bodyHeight * .45);
	...
	const outline = [
		...generateCirclePoints(0, -headCenterY, headRadius, 24, 180 + phiRightDeg, -phiRightDeg),
		...generateCirclePoints(-(-w / 2 + bodyRadius), -(bodyTop + bodyRadius), bodyRadius, 12, 90, 0),
		...generateCirclePoints(-(-w / 2 + bodyRadius), -(totalHeight / 2 - bodyRadius), bodyRadius, 12, 360, 270),
		...generateCirclePoints(-(w / 2 - bodyRadius), -(totalHeight / 2 - bodyRadius), bodyRadius, 12, 270, 180),
		...generateCirclePoints(-(w / 2 - bodyRadius), -(bodyTop + bodyRadius), bodyRadius, 12, 180, 90)
	];
	node.intersect = function(point) {
		return intersect_default.polygon(node, outline, point);
	};
...
async function drawRect(parent, node, options) {
	...
	updateNodeBounds(node, rect, node.look === "handDrawn" ? void 0 : {
		width: totalWidth,
		height: totalHeight
	});
	node.calcIntersect = function(bounds, point) {
...
async function datastore(parent, node) {
	...
	const rect = await drawRect(parent, node, {
		rx: 0,
		ry: 0,
		classes: cssClasses ?? "",
	...
	if (node.look === "handDrawn") {
		...
		if (!borderPath) return rect;
		let bbox = null;
		if (borderPath instanceof SVGGraphicsElement) bbox = borderPath.getBBox();
		else return rect;
		rect.insert(() => rc.line(bbox.x, bbox.y, bbox.x + bbox.width, bbox.y, nodeOverrideOptions), ".basic.label-container g.label");
		rect.insert(() => rc.line(bbox.x, bbox.y + bbox.height, bbox.x + bbox.width, bbox.y + bbox.height, nodeOverrideOptions), ".basic.label-container g.label");
...
async function usecaseEllipse(parent, node) {
	...
	if (node.look === "handDrawn") {
		...
		if (cssStyles) ellipseElem.attr("style", cssStyles);
	} else ellipseElem = shapeSvg.insert("ellipse", ":first-child").attr("class", "basic label-container").attr("style", nodeStyles).attr("rx", radiusX).attr("ry", radiusY).attr("cx", 0).attr("cy", 0);
...
async function folder(parent, node) {
	...
	let folderShape;
	if (node.look === "handDrawn") {
		const roughNode = at.svg(shapeSvg).path(pathData, userNodeOverrides(node, {}));
	...
	if (node.look === "handDrawn") updateNodeBounds(node, folderShape);
	else updateNodeBounds(node, folderShape, {
		width: w,
		height: totalHeight
	});
...
async function halfRoundedRectangle(parent, node) {
	...
	const radius = h / 2;
	const capClearance = radius - Math.sqrt(Math.max(0, radius ** 2 - (bbox.height / 2) ** 2));
	const w = (node?.width ? node?.width : Math.max(minWidth, bbox.width) + capClearance * 2) + paddingX * 2;
	const { cssStyles } = node;
	const rc = at.svg(shapeSvg);
	const options = userNodeOverrides(node, {});
	if (node.look !== "handDrawn") {
...
async function labelRect(parent, node) {
	...
	const rect = shapeSvg.insert("rect", ":first-child");
	rect.attr("width", .1).attr("height", .1);
...
async function linedWaveEdgedRect(parent, node) {
	...
	label.attr("transform", `translate(${-w / 2 + (node.padding ?? 0) + w / 2 * .1 / 2 - (bbox.x - (bbox.left ?? 0))},${-h / 2 + (node.padding ?? 0) - waveAmplitude - (bbox.y - (bbox.top ?? 0))})`);
...
var BUSINESS_MARKER_OFFSET_RATIO = .6;
var businessMarkerPathForCircle = /* @__PURE__ */ __name((centerY, radius) => {
...
var positionLabel = /* @__PURE__ */ __name((label, bbox, centerY) => {
	const originX = (bbox.x ?? 0) - (bbox.left ?? 0);
	const originY = (bbox.y ?? 0) - (bbox.top ?? 0);
	label.attr("transform", `translate(${-bbox.width / 2 - originX},${centerY - bbox.height / 2 - originY})`);
}, "positionLabel");
var appendActorPath = /* @__PURE__ */ __name((group, node, pathData, className, hollow = false) => {
	if (node.look === "handDrawn") {
		const path2 = at.svg(group).path(pathData, userNodeOverrides(node, hollow ? { fill: "none" } : {}));
	...
	if (hollow) path.attr("fill", "none");
	return path;
}, "appendActorPath");
var appendActorCircle = /* @__PURE__ */ __name((group, node, cx, cy, radius, className, hollow = false) => {
	if (node.look === "handDrawn") {
		const circle3 = at.svg(group).circle(cx, cy, radius * 2, userNodeOverrides(node, hollow ? { fill: "none" } : {}));
	...
	if (hollow) circle2.attr("fill", "none");
	return circle2;
}, "appendActorCircle");
var appendBusinessActorMarker = /* @__PURE__ */ __name((group, node, variant) => {
...
async function renderUsecaseActor(parent, node, variant, drawGlyph) {
	const { labelStyles, nodeStyles } = styles2String(node);
	node.labelStyle = labelStyles;
	const { shapeSvg, bbox: labelBox, label } = await labelHelper(parent, variant === "icon" || node.stereotype !== void 0 ? {
		...node,
		icon: variant === "icon" ? void 0 : node.icon,
		stereotype: void 0
	} : node, getNodeClasses(node, `usecase-actor-variant usecase-actor-${variant}`));
	const accessibleName = node.accessibleName ?? node.label;
	if (accessibleName) shapeSvg.attr("role", "img").attr("aria-label", accessibleName);
	...
	let stereotypeBox;
	if (node.stereotype) {
		const stereotype = await insertLabel(shapeSvg, `\xAB${node.stereotype}\xBB`, {
	...
	const totalWidth = Math.max(ACTOR_FIGURE_WIDTH, labelBox.width, stereotypeBox?.width ?? 0) + padding * 2;
	...
	updateNodeBounds(node, shapeSvg.insert("rect", ":first-child").attr("class", "usecase-actor-outline").attr("x", -totalWidth / 2).attr("y", -totalHeight / 2).attr("width", totalWidth).attr("height", totalHeight).attr("opacity", 0).attr("aria-hidden", "true"));
...
var ICON_SIZE = 42;
var ICON_CENTER_Y = -2;
var drawIconActor = /* @__PURE__ */ __name(async (group, node) => {
	const frameX = -ICON_FRAME_SIZE / 2;
	const frameY = ICON_CENTER_Y - ICON_FRAME_SIZE / 2;
	if (node.look === "handDrawn") {
		const frame = at.svg(group).rectangle(frameX, frameY, ICON_FRAME_SIZE, ICON_FRAME_SIZE, userNodeOverrides(node, {}));
		group.insert(() => frame, ":first-child").attr("class", "usecase-actor-icon-frame");
	} else group.append("rect").attr("class", "usecase-actor-icon-frame").attr("x", frameX).attr("y", frameY).attr("width", ICON_FRAME_SIZE).attr("height", ICON_FRAME_SIZE).attr("rx", 4).attr("ry", 4);
	...
	if (iconBox) iconGroup.attr("transform", `translate(${-iconBox.width / 2 - iconBox.x},${ICON_CENTER_Y - iconBox.height / 2 - iconBox.y})`);
}, "drawIconActor");
...
var positionLabel2 = /* @__PURE__ */ __name((label, bbox, centerY) => {
	const viewportOffsetX = (bbox.x ?? 0) - (bbox.left ?? 0);
	const viewportOffsetY = (bbox.y ?? 0) - (bbox.top ?? 0);
	label.attr("transform", `translate(${-bbox.width / 2 - viewportOffsetX},${centerY - bbox.height / 2 - viewportOffsetY})`);
}, "positionLabel");
async function usecaseBusiness(parent, node) {
	const businessNode = node;
	const { labelStyles, nodeStyles } = styles2String(node);
	node.labelStyle = labelStyles;
	const { shapeSvg, bbox: labelBox, halfPadding, label } = await labelHelper(parent, businessNode.stereotype ? {
		...node,
		stereotype: void 0
	} : node, getNodeClasses(node, "usecase-business-shape"));
	...
	if (businessNode.stereotype) {
		const stereotype = await insertLabel(shapeSvg, `\xAB${businessNode.stereotype}\xBB`, {
			labelStyle: labelStyles,
			useHtmlLabels: node.useHtmlLabels,
			padding: 0,
			centerLabel: true,
	...
	if (stereotypeLabel && stereotypeBox) positionLabel2(stereotypeLabel, stereotypeBox, -labelHeight / 2 + stereotypeBox.height / 2);
	positionLabel2(label, labelBox, labelHeight / 2 - labelBox.height / 2);
	let ellipseElement;
	if (node.look === "handDrawn") {
		const roughEllipse = at.svg(shapeSvg).ellipse(0, 0, radiusX * 2, radiusY * 2, userNodeOverrides(node, {}));
		ellipseElement = shapeSvg.insert(() => roughEllipse, ":first-child").attr("class", "basic label-container usecase-business-ellipse");
	} else ellipseElement = shapeSvg.insert("ellipse", ":first-child").attr("class", "basic label-container usecase-business-ellipse").attr("style", nodeStyles).attr("rx", radiusX).attr("ry", radiusY).attr("cx", 0).attr("cy", 0);
...
var positionLabel3 = /* @__PURE__ */ __name((label, bbox, centerX, centerY) => {
	const originX = (bbox.x ?? 0) - (bbox.left ?? 0);
	const originY = (bbox.y ?? 0) - (bbox.top ?? 0);
	label.attr("transform", `translate(${centerX - bbox.width / 2 - originX},${centerY - bbox.height / 2 - originY})`);
}, "positionLabel");
var numericStrokeWidth = /* @__PURE__ */ __name((value) => {
...
async function usecaseJsonTable(parent, node) {
	...
	const { stylesMap } = compileStyles(node);
	node.labelStyle = labelStyles;
	const { shapeSvg, bbox: titleBox, label: titleLabel } = await labelHelper(parent, node, getNodeClasses(node, "usecase-json-table"));
	shapeSvg.attr("role", "img");
	if (tableNode.accessibleName) shapeSvg.attr("aria-label", tableNode.accessibleName);
	...
	const rowHeights = measuredRows.map(({ key, value }) => Math.max(key.bbox.height, value.bbox.height) + CELL_PADDING_Y * 2);
	...
	const insertCell = /* @__PURE__ */ __name((container, className, x, y, width, height) => {
	...
	const outerBorder = insertCell(shapeSvg, "label-container usecase-json-border", -totalWidth / 2, -totalHeight / 2, totalWidth, totalHeight);
	insertCell(tableGroup, "usecase-json-cell usecase-json-title-cell", left, top, innerWidth, titleHeight);
	...
	measuredRows.forEach(({ key, value }, index) => {
		...
		insertCell(rowGroup, "usecase-json-cell usecase-json-value-cell", left + keyWidth, 0, valueWidth, rowHeight);
...
async function multiWaveEdgedRectangle(parent, node) {
	...
	label.attr("transform", `translate(${-(bbox.width / 2) - rectOffset2 - (bbox.x - (bbox.left ?? 0))}, ${-(bbox.height / 2) + rectOffset2 - waveAmplitude - (bbox.y - (bbox.top ?? 0))})`);
	updateNodeBounds(node, shape);
	node.intersect = function(point) {
		return intersect_default.polygon(node, outerPathPoints, point);
	};
...
async function note(parent, node, { config: { themeVariables } }) {
	...
	if (cssStyles && node.look !== "handDrawn") rect.selectAll("path").attr("style", cssStyles);
	if (nodeStyles && node.look !== "handDrawn") rect.selectAll("path").attr("style", nodeStyles);
	label.attr("transform", `translate(${-bbox.width / 2 - (bbox.x - (bbox.left ?? 0))}, ${-(bbox.height / 2) - (bbox.y - (bbox.top ?? 0))})`);
	updateNodeBounds(node, rect);
	node.intersect = function(point) {
		return intersect_default.rect(node, point);
...
async function rect_left_inv_arrow(parent, node) {
	...
	const h = Math.max(bbox.height + (node.look === "neo" ? labelPaddingY * 2 : labelPaddingY), node.height ?? 0);
	const notchWidth = h / 4;
	const x = -Math.max(labelWidth, (node.width ?? 0) - notchWidth) / 2;
	const y = -h / 2;
	const notch = y / 2;
	const points = [
		{
...
async function shadedProcess(parent, node) {
	...
	if (nodeStyles && node.look !== "handDrawn") rect.selectAll("path").attr("style", nodeStyles);
	if (cssStyles && node.look !== "handDrawn") rect.selectAll("path").attr("style", nodeStyles);
...
async function squareRect(parent, node) {
	...
	const labelPaddingY = node.look === "neo" ? 12 : nodePadding;
	const shapeSvg = await drawRect(parent, node, {
		rx: 0,
		ry: 0,
		classes: "",
...
var stadiumDimensions = /* @__PURE__ */ __name((bbox, paddingX, paddingY) => {
	const h = bbox.height + paddingY;
	...
	return {
		w: Math.max(bbox.width + h / 4 + paddingX, 1.5 * h + (h - inscribedDiameter), bbox.width + paddingX + h - capWidthAtLabel),
		h
	};
}, "stadiumDimensions");
...
async function triangle(parent, node) {
	...
	if (node.width || node.height) {
		node.width = ((node?.width ?? 0) - labelPaddingX) / 2;
		if (node.width < MIN_WIDTH6) node.width = MIN_WIDTH6;
		node.height = node?.height ?? 0;
...
async function requirementBox(parent, node) {
	...
	if (isRequirementNode) {
		const idHeight = await addText3(shapeSvg, `${requirementNode.requirementId ? `ID: ${requirementNode.requirementId}` : ""}`, accumulativeHeight, node.labelStyle, bodyTextAlignment);
		accumulativeHeight += idHeight;
		const textHeight = await addText3(shapeSvg, `${requirementNode.text ? `Text: ${requirementNode.text}` : ""}`, accumulativeHeight, node.labelStyle, bodyTextAlignment);
		accumulativeHeight += textHeight;
		const riskHeight = await addText3(shapeSvg, `${requirementNode.risk ? `Risk: ${requirementNode.risk}` : ""}`, accumulativeHeight, node.labelStyle, bodyTextAlignment);
		accumulativeHeight += riskHeight;
		await addText3(shapeSvg, `${requirementNode.verifyMethod ? `Verification: ${requirementNode.verifyMethod}` : ""}`, accumulativeHeight, node.labelStyle, bodyTextAlignment);
	} else {
		const typeHeight2 = await addText3(shapeSvg, `${elementNode.type ? `Type: ${elementNode.type}` : ""}`, accumulativeHeight, node.labelStyle, bodyTextAlignment);
		accumulativeHeight += typeHeight2;
		await addText3(shapeSvg, `${elementNode.docRef ? `Doc Ref: ${elementNode.docRef}` : ""}`, accumulativeHeight, node.labelStyle, bodyTextAlignment);
	}
...
async function addText3(parentGroup, inputText, yOffset, style = "", alignment = "center") {
	...
	if (!useHtmlLabels) {
		...
		if (alignment === "start") {
			...
			for (const child of textChild.children) child.setAttribute("text-anchor", "start");
		}
		bbox = text2.getBBox();
		bbox.height += 6;
	} else {
		const div = text2.children[0];
		const dv = select_default(text2);
		if (alignment === "start") select_default(div).style("text-align", "left");
		bbox = div.getBoundingClientRect();
		dv.attr("width", bbox.width);
		dv.attr("height", bbox.height);
	}
...
//#endregion
export { isValidShape as a, updateNodeBounds as c, intersect_rect_default as i, createRoundedRectPathD as n, labelHelper as o, intersect_default as r, shapes as s, createLabel_default as t };
