...
import { H as setAccDescription, U as setAccTitle, a as clear, at as rgba, q as setDiagramTitle, s as common_default, v as getAccDescription, w as getDiagramTitle, x as getConfig2, y as getAccTitle } from "./chunk-O7XYJQB3-mKDgYvQm.js";
...
import { a as paletteSlotCount, i as isColorTheme, o as safeLook, r as hasPalette, t as COLOR_THEMES } from "./chunk-J5ZVWO5B-DBdtyzdO.js";
...
//#region node_modules/.pnpm/mermaid@12.0.0/node_modules/mermaid/dist/chunks/mermaid.core/erDiagram-OPXOYQCR.mjs
var parser = (function() {
	var o = /* @__PURE__ */ __name(function(k, v, o2, l) {
		...
		54,
		55
	], $V1 = [2, 2], $V2 = [1, 7], $V3 = [1, 9], $V4 = [1, 10], $V5 = [1, 11], $V6 = [1, 12], $V7 = [1, 30], $V8 = [1, 23], $V9 = [1, 24], $Va = [1, 25], $Vb = [1, 26], $Vc = [1, 27], $Vd = [1, 19], $Ve = [1, 28], $Vf = [1, 29], $Vg = [1, 20], $Vh = [1, 18], $Vi = [1, 21], $Vj = [1, 22], $Vk = [2, 6], $Vl = [
		6,
		9,
		21,
		23,
		...
		54,
		55
	], $Vm = [1, 36], $Vn = [1, 37], $Vo = [1, 38], $Vp = [1, 39], $Vq = [1, 40], $Vr = [
		6,
		9,
		12,
		14,
		...
		74,
		75
	], $Vw = [1, 70], $Vx = [1, 68], $Vy = [1, 65], $Vz = [1, 69], $VA = [1, 71], $VB = [
		6,
		9,
		12,
		16,
		...
		51,
		52
	], $VJ = [1, 101], $VK = [1, 100], $VL = [1, 99], $VM = [18, 61], $VN = [1, 110], $VO = [1, 109], $VP = [
		20,
		43,
		51,
		53,
		...
		performAction: /* @__PURE__ */ __name(function anonymous(yytext, yyleng, yylineno, yy, yystate, $$, _$) {
			...
			switch (yystate) {
				...
				case 82:
					...
					break;
				case 83: this.$ = yy.Identification.IDENTIFYING;
			}
		}, "anonymous"),
		table: [
			{
...
var ErDB = class {
	...
	*/
	addRelationship(entA, rolA, entB, rSpec) {
		let entityAId;
		if (this.subGraphLookup.has(entA)) entityAId = entA;
		else {
		...
		let entityBId;
		if (this.subGraphLookup.has(entB)) entityBId = entB;
		else {
	...
	addCssStyles(ids, styles) {
		for (const id of ids) {
			...
			if (subGraph) {
				if (!subGraph.cssStyles) subGraph.cssStyles = [];
				for (const style of styles) subGraph.cssStyles.push(style);
			}
		}
	}
	addClass(ids, style) {
	...
	addSubGraph(_id, list, _title) {
		...
		const result = (/* @__PURE__ */ __name((a) => {
			...
			let dir2;
			return {
				nodeList: a.filter((item) => {
					if (item?.stmt) {
						if (item.stmt === "dir") dir2 = item.value;
						return false;
					}
	...
	setClass(ids, classNames) {
		for (const id of ids) {
			...
			const subGraph = this.subGraphLookup.get(id);
			if (subGraph) for (const className of classNames) subGraph.classes.push(className);
		}
	}
	/**
	...
	*/
	makeUniq(subGraph, allSubgraphs) {
		...
		subGraph.nodes.forEach((_id, pos) => {
			if (existingNodes.has(_id)) log.warn(`Entity '${_id}' already belongs to another subgraph and will be ignored`);
			else res.push(subGraph.nodes[pos]);
		});
		return { nodes: res };
	}
	sanitizeText(txt) {
	...
	getData() {
		...
		for (const entityKey of this.entities.keys()) {
			if (subGraphIds.has(entityKey)) continue;
