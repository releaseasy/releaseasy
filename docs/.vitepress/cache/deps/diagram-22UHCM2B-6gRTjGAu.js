...
import { G as setConfig2, H as setAccDescription, U as setAccTitle, a as clear, at as rgba, d as defaultConfig2, et as purify, q as setDiagramTitle, s as common_default, v as getAccDescription, w as getDiagramTitle, x as getConfig2, y as getAccTitle } from "./chunk-O7XYJQB3-mKDgYvQm.js";
import { t as channel } from "./channel-DXKY7-XB.js";
import { c as getEdgeId, g as utils_default } from "./chunk-ZIGJFQKS-CX9G_8OE.js";
...
import { i as isColorTheme, n as colorSlotCount, o as safeLook, r as hasPalette } from "./chunk-J5ZVWO5B-DBdtyzdO.js";
import { a as isValidShape } from "./chunk-7INBJB4K-Cqr6ftBk.js";
...
import { t as getIconStyles } from "./chunk-5VM5RSS4-CB1tztU3.js";
import { t as createTooltip } from "./chunk-GWA4HPMP-BXj13iAN.js";
...
//#region node_modules/.pnpm/mermaid@12.0.0/node_modules/mermaid/dist/chunks/mermaid.core/diagram-22UHCM2B.mjs
...
new Set(Object.values(AgentflowWarning).filter((id) => !PARSER_EMITTED_DIAGNOSTICS.has(id)));
...
function normaliseNodeShapes(nodes, sink) {
	for (const node of nodes) {
		...
		if (REMOVED_SHAPES.has(node.shape)) {
			...
			if (sink?.emitError) sink.emitError("SHAPE_REMOVED", msg, { nodeId: node.id });
			else log.warn(`agentflow: ${msg}`);
			node.shape = DEFAULT_SHAPE;
			continue;
		}
		if (!ALLOWED_SHAPES.has(node.shape)) {
			const msg = `shape "${node.shape}" is not supported, using "${DEFAULT_SHAPE}"`;
			if (sink?.emitWarning) sink.emitWarning("SHAPE_UNSUPPORTED", msg, { nodeId: node.id });
			else log.warn(`agentflow: ${msg}`);
			node.shape = DEFAULT_SHAPE;
		}
	}
...
var kindClass = /* @__PURE__ */ __name((kind) => `af-kind-${kind}`, "kindClass");
var KINDS = [...KIND_SLOT.keys()];
var containerSlotCount = /* @__PURE__ */ __name((paletteLength) => Math.max(1, paletteLength - KIND_COUNT), "containerSlotCount");
var containerSlot = /* @__PURE__ */ __name((n, paletteLength) => {
...
function assignColorSlots(nodes, kindOf, containerOrder) {
	const palette = getConfig2().themeVariables?.borderColorArray;
	...
	for (const node of nodes ?? []) {
		if (node.isGroup || node.shape === "collapsedGroup") {
			node.colorIndex = containerSlot(containerOrder.get(String(node.id)) ?? fallbackOrdinal++, paletteLength);
			continue;
		}
		const kind = kindOf(String(node.id));
		if (kind && KIND_SLOT.has(kind)) node.cssClasses = `${node.cssClasses ?? ""} ${kindClass(kind)}`.replace(/\s+/g, " ").trim();
	}
}
...
var SUBGRAPH_TYPE_TO_SHAPE = { flow: "flowGroup" };
function stripPrototypeKeys(value) {
	if (Array.isArray(value)) return value.map((entry) => stripPrototypeKeys(entry));
...
var AgentFlowDB = class {
	...
	addVertex(id, textObj, type, style, classes, dir, props = {}, metadata, metadataLoc) {
		...
		if (metadata !== void 0) {
			...
			try {
				...
				if (stripped !== metadata) try {
					doc = load(stripped + "\n", { schema: JSON_SCHEMA }) ?? {};
				} catch {
					this.rethrowMetadataYamlError(err, metadata, metadataLoc);
				}
				else this.rethrowMetadataYamlError(err, metadata, metadataLoc);
			}
		...
		if (doc && typeof doc.shape === "string") {
			...
			if (resolved && resolved !== authoredShape) docRec.shape = resolved;
		}
		...
		if (doc) {
			...
			if (doc.shape) {
				...
				if (typeof authored !== "string") throw this.positionedShapeError(`No such shape: ${JSON.stringify(authored)}.`, metadataLoc);
				if (authored !== authored.toLowerCase() || authored.includes("_")) throw this.positionedShapeError(`No such shape: ${authored}. Shape names should be lowercase.`, metadataLoc);
				else if (!isValidShape(doc.shape)) throw this.positionedShapeError(`No such shape: ${doc.shape}.`, metadataLoc);
	...
	*/
	...
	rethrowMetadataYamlError(err, metadata, metadataLoc) {
		const ex = err;
		if (!ex || ex.name !== "YAMLException" || !ex.mark || !metadataLoc) throw err;
		let translation;
		try {
			...
			const toSource = /* @__PURE__ */ __name((line, column) => {
				if (isInline) return {
					line: blockLine,
					column: contentColumn + (line === 1 ? column : 0)
				};
				return {
					line: blockLine + line,
			...
			const display = [];
			if (isInline) display.push(1);
			else for (let i = ex.mark.line - 1; i <= ex.mark.line + 1; i++) {
				if (i < 0 || i >= bufferLines.length) continue;
				if (i === bufferLines.length - 1 && bufferLines[i] === "" && i !== ex.mark.line) continue;
				display.push(i);
			}
			...
			for (const i of display) {
				...
				if (i === ex.mark.line) snippet.push("-".repeat(prefix.length + ex.mark.column) + "^");
			}
			translation = {
...
You have to call mermaid.initialize.`);
	...
	*/
	getTools() {
		const tools = [];
		for (const vertex of this.vertices.values()) if (this.isToolDefinition(vertex)) tools.push(vertex);
		return tools;
	}
	/**
	...
	*/
	...
	addSubGraph(_id, list, _title, type) {
		...
		if (_id === _title && _title?.text && /\s/.exec(_title.text)) id = void 0;
		const result = (/* @__PURE__ */ __name((a) => {
			...
			return {
				nodeList: a.filter(function(item) {
					...
					if (type2 in prims) return prims[type2].hasOwnProperty(item) ? false : prims[type2][item] = true;
		...
		if (existingPos !== -1) {
			...
			for (const nodeId of subGraph.nodes) if (!existing.nodes.includes(nodeId)) existing.nodes.push(nodeId);
			if (subGraph.title) existing.title = subGraph.title;
			if (subGraph.dir) existing.dir = subGraph.dir;
			if (subGraph.type) existing.type = subGraph.type;
	...
	*/
	addGlobal(list) {
		for (const item of list.flat()) {
			...
			const id = item.trim();
			if (id !== "") this.globalNodes.add(id);
		}
		for (const subGraph of this.subGraphs) subGraph.nodes = subGraph.nodes.filter((nodeId) => !this.globalNodes.has(nodeId));
	}
	getPosForId(id) {
	...
	*/
	addConnector(textObj, titleObj) {
		...
		if (!connector) {
			...
			this.vertexCounter++;
		} else if (title.length > 0) connector.text = resolvedTitle;
		return id;
	}
	/**
	...
	*/
	runPostParseValidators() {
		if (this.postParseValidationRun) return;
	...
	getData() {
		...
		const collectDescendants = /* @__PURE__ */ __name((sgId, ancestor, seen = /* @__PURE__ */ new Set()) => {
			if (seen.has(sgId)) return;
			seen.add(sgId);
			const sg = this.subGraphLookup.get(sgId);
			if (!sg) return;
			for (const childId of sg.nodes) {
				if (childId === ancestor || seen.has(childId)) continue;
				hiddenIds.add(childId);
				collapsedAncestorMap.set(childId, ancestor);
				collectDescendants(childId, ancestor, seen);
			}
		}, "collectDescendants");
		for (const sg of subGraphs) if (sg.metadata?.view === "collapsed" && !hiddenIds.has(sg.id)) collectDescendants(sg.id, sg.id);
		const wouldCloseContainmentCycle = /* @__PURE__ */ __name((childId, parentId) => {
		...
		for (let i = subGraphs.length - 1; i >= 0; i--) {
			...
			if (hiddenIds.has(subGraph.id)) continue;
			if (subGraph.nodes.length > 0) subGraphDB.set(subGraph.id, true);
			for (const id of subGraph.nodes) {
				if (wouldCloseContainmentCycle(id, subGraph.id)) {
					this.emitWarning(AgentflowWarning.CONTAINMENT_VIOLATION, `Container "${subGraph.id}" cannot contain "${id}" because "${id}" already contains it. The nesting that would close the loop is dropped.`, { nodeId: id });
					continue;
				}
		...
		for (const sg of subGraphs) {
			...
			if (parent !== void 0) childContainers.set(parent, [...childContainers.get(parent) ?? [], sg.id]);
		}
		let nextContainer = 0;
		const walkContainers = /* @__PURE__ */ __name((id) => {
			if (containerOrder.has(id)) return;
			containerOrder.set(id, nextContainer++);
			for (const child of childContainers.get(id) ?? []) walkContainers(child);
		}, "walkContainers");
		for (const sg of subGraphs) if (parentDB.get(sg.id) === void 0) walkContainers(sg.id);
		const connectorIds = new Set(this.getConnectors().map((c) => c.id));
		assignColorSlots(nodes, (id) => {
			if (connectorIds.has(id)) return "connector";
	...
	*/
	deriveVertexKind(v, resolvedShape) {
		if (this.isToolDefinition(v)) return "tool";
		if (resolvedShape === "hexagon" || resolvedShape === "hex") return "action";
		if (resolvedShape === "lean-right" || resolvedShape === "lean_right") return "input";
		if (resolvedShape === "lin-doc" || resolvedShape === "lined-document") return "refdoc";
		if (resolvedShape === "diamond") return "decision";
		return "task";
	}
	getSemanticModel() {
		this.runPostParseValidators();
		...
		for (const [id, v] of this.vertices) {
			...
			if (v.isConnector) {
				const connector = { id };
				if (v.text !== void 0 && v.text !== id) connector.title = v.text;
				if (v.metadata && Object.keys(v.metadata).length > 0) {
					const meta = {};
					for (const [key, value] of Object.entries(v.metadata)) {
						if (SEMANTIC_METADATA_SKIP_KEYS.has(key)) continue;
						meta[key] = value;
					}
					if (Object.keys(meta).length > 0) connector.metadata = meta;
				}
				connectors.push(connector);
				continue;
			}
			const vertex = { id };
			if (v.text !== void 0) vertex.label = v.text;
			const resolvedShape = typeof v.type === "string" ? resolveShapeAlias(v.type) : v.type;
			if (resolvedShape !== void 0) vertex.shape = resolvedShape;
			...
			if (v.metadata && Object.keys(v.metadata).length > 0) {
				...
				if (Object.keys(meta).length > 0) vertex.metadata = meta;
			}
		...
		const model = {
			...
			edges: this.edges.map((e) => {
				...
				if (e.id !== void 0) edge.id = e.id;
				if (typeof e.text === "string" && e.text.length > 0) edge.label = e.text;
				if (e.type !== void 0) edge.type = e.type;
				if (e.stroke !== void 0) edge.stroke = e.stroke;
				if (e.edgeSemantic !== void 0) edge.edgeSemantic = e.edgeSemantic;
				if (e.length !== void 0) edge.length = e.length;
				if (e.metadata && Object.keys(e.metadata).length > 0) edge.metadata = { ...e.metadata };
				return edge;
			}),
			subGraphs: this.subGraphs.map((sg) => {
				const out = {
					id: sg.id,
					nodes: [...sg.nodes]
				};
				if (sg.type !== void 0) out.type = sg.type;
				if (sg.title !== void 0) out.title = sg.title;
				if (sg.dir !== void 0) out.direction = sg.dir;
		...
		if (this.direction !== void 0) model.direction = this.direction;
		return model;
	}
	...
	addVertexMapping(id, _text, shape, loc) {
		...
		if (mapping && shape == null) this.bareVertexMappings.add(mapping);
	}
	/**
	...
	*/
	extendVertexMapping(id, loc) {
		...
		for (let i = this.elementMappings.length - 1; i >= 0; i--) {
			...
			if (m.type === "vertex" && m.id === id) {
				...
				if (end.endLine > m.position.endLine || end.endLine === m.position.endLine && end.endColumn > m.position.endColumn) {
	...
	addEdgeMapping(_fromStmt, toNodes, _link, loc) {
		const ids = Array.isArray(toNodes) ? toNodes.map((n) => typeof n === "string" ? n : n?.id ?? "").filter(Boolean) : [];
		const edgeId = ids.length > 0 ? ids.join(">") : "edge";
		this.pushMapping(edgeId, "edge", loc);
	}
	addSubgraphMapping(_id, _title, startLoc, endLoc) {
		const id = (typeof _id === "string" ? _id : _id?.text) ?? "";
	...
	getElementsOnLine(line) {
		return this.elementMappings.filter((m) => line >= m.position.startLine && line <= m.position.endLine);
	}
	getElementAtPosition(line, column) {
		...
		const span = /* @__PURE__ */ __name(({ position }) => [position.endLine - position.startLine, position.endColumn - position.startColumn], "span");
		return candidates.reduce((smallest, cur) => {
			const [smallLines, smallColumns] = span(smallest);
			const [curLines, curColumns] = span(cur);
			if (curLines !== smallLines) return curLines < smallLines ? cur : smallest;
	...
	getMappingStats() {
		...
		let connectors = 0;
		let attachments = 0;
		for (const m of this.elementMappings) if (m.type === "vertex") vertices++;
		else if (m.type === "edge") edges++;
		else if (m.type === "subgraph") subgraphs++;
		else if (m.type === "connector") connectors++;
		else if (m.type === "attachment") attachments++;
		return {
			vertices,
			edges,
			subgraphs,
	...
	emitDiagnostic(id, severity, message, ctx) {
		...
		if (this.diagnostics.some((existing) => existing.id === diagnostic.id && existing.severity === diagnostic.severity && existing.message === diagnostic.message && existing.nodeId === diagnostic.nodeId && existing.edgeId === diagnostic.edgeId)) return;
		this.diagnostics.push(diagnostic);
		const formatted = `agentflow[${id}]: ${message}`;
		if (severity === "error") log.error(formatted);
...
var renderer_default = {
	...
	draw: /* @__PURE__ */ __name(async function(text, id, _version, diag) {
		...
		utils_default.insertTitle(svg, "agentflowTitleText", conf?.titleTopMargin ?? 0, diag.db.getDiagramTitle());
...
var parser = (function() {
	var o = /* @__PURE__ */ __name(function(k, v, o2, l) {
		...
		return o2;
	}, "o"), $V0 = [1, 5], $V1 = [1, 4], $V2 = [1, 3], $V3 = [1, 6], $V4 = [
		1,
		8,
		9,
		10,
		...
		127,
		128
	], $V5 = [2, 2], $V6 = [1, 15], $V7 = [1, 16], $V8 = [1, 17], $V9 = [1, 18], $Va = [1, 19], $Vb = [1, 26], $Vc = [1, 27], $Vd = [1, 28], $Ve = [1, 30], $Vf = [1, 31], $Vg = [1, 32], $Vh = [1, 55], $Vi = [1, 54], $Vj = [1, 34], $Vk = [1, 35], $Vl = [1, 36], $Vm = [1, 37], $Vn = [1, 38], $Vo = [1, 50], $Vp = [1, 52], $Vq = [1, 48], $Vr = [1, 53], $Vs = [1, 49], $Vt = [1, 56], $Vu = [1, 51], $Vv = [1, 57], $Vw = [1, 58], $Vx = [1, 39], $Vy = [1, 40], $Vz = [1, 41], $VA = [1, 42], $VB = [1, 43], $VC = [1, 64], $VD = [
		1,
		8,
		9,
		10,
		...
		127,
		128
	], $VE = [1, 70], $VF = [1, 68], $VG = [1, 67], $VH = [1, 69], $VI = [
		8,
		9,
		10,
		12,
		...
		80,
		81
	], $VJ = [1, 88], $VK = [1, 101], $VL = [1, 106], $VM = [1, 105], $VN = [1, 102], $VO = [1, 98], $VP = [1, 104], $VQ = [1, 100], $VR = [1, 107], $VS = [1, 103], $VT = [1, 108], $VU = [1, 99], $VV = [
		8,
		9,
		10,
		11,
		...
		118,
		119
	], $V_ = [1, 131], $V$ = [1, 132], $V01 = [1, 134], $V11 = [1, 133], $V21 = [
		47,
		63,
		65,
		77,
		...
		118,
		119
	], $V31 = [1, 143], $V41 = [1, 157], $V51 = [1, 158], $V61 = [1, 159], $V71 = [1, 162], $V81 = [1, 161], $V91 = [1, 160], $Va1 = [1, 145], $Vb1 = [1, 147], $Vc1 = [1, 151], $Vd1 = [1, 139], $Ve1 = [1, 140], $Vf1 = [1, 152], $Vg1 = [1, 153], $Vh1 = [1, 154], $Vi1 = [1, 155], $Vj1 = [1, 156], $Vk1 = [1, 163], $Vl1 = [1, 164], $Vm1 = [1, 141], $Vn1 = [1, 142], $Vo1 = [1, 149], $Vp1 = [1, 144], $Vq1 = [1, 148], $Vr1 = [1, 146], $Vs1 = [
		8,
		9,
		10,
		11,
		...
		118,
		119
	], $Vx1 = [1, 190], $Vy1 = [1, 186], $Vz1 = [1, 187], $VA1 = [1, 191], $VB1 = [1, 188], $VC1 = [1, 189], $VD1 = [
		80,
		119,
		122
	], $VE1 = [
		...
		120,
		121
	], $VH1 = [1, 269], $VI1 = [1, 267], $VJ1 = [1, 271], $VK1 = [1, 265], $VL1 = [1, 266], $VM1 = [1, 268], $VN1 = [1, 270], $VO1 = [1, 272], $VP1 = [1, 292], $VQ1 = [
		8,
		9,
		10,
		12,
	...
	var parser2 = {
		...
		symbols_: {
			...
			"separator": 22,
			"styleStatement": 23,
			"linkStyleStatement": 24,
			"classDefStatement": 25,
			"classStatement": 26,
			"clickStatement": 27,
			"flow": 28,
			"textNoTags": 29,
			...
			"SUBROUTINESTART": 59,
			"SUBROUTINEEND": 60,
			"VERTEX_WITH_PROPS_START": 61,
			"NODE_STRING[field]": 62,
			"COLON": 63,
			"NODE_STRING[value]": 64,
			"PIPE": 65,
			"CYLINDERSTART": 66,
			"CYLINDEREND": 67,
			"DIAMOND_START": 68,
		...
		terminals_: {
			...
			40: "acc_descr",
			41: "acc_descr_value",
			42: "acc_descr_multiline_value",
			43: "SHAPE_DATA",
			47: "AMP",
			49: "STYLE_SEPARATOR",
			51: "DOUBLECIRCLESTART",
			52: "DOUBLECIRCLEEND",
			53: "PS",
			54: "PE",
			55: "(-",
			...
			59: "SUBROUTINESTART",
			60: "SUBROUTINEEND",
			61: "VERTEX_WITH_PROPS_START",
			62: "NODE_STRING[field]",
			63: "COLON",
			64: "NODE_STRING[value]",
			65: "PIPE",
			66: "CYLINDERSTART",
			67: "CYLINDEREND",
			68: "DIAMOND_START",
		...
		performAction: /* @__PURE__ */ __name(function anonymous(yytext, yyleng, yylineno, yy, yystate, $$, _$) {
			...
			switch (yystate) {
				...
				case 38:
					...
					if (yy.addSubgraphMapping) yy.addSubgraphMapping($$[$0 - 6], $$[$0 - 4], _$[$0 - 8], _$[$0]);
					break;
				case 39:
					this.$ = yy.addSubGraph($$[$0 - 7], $$[$0 - 1], $$[$0 - 5], "flow");
					yy.addVertex(this.$, void 0, void 0, void 0, void 0, void 0, void 0, $$[$0 - 3], _$[$0 - 3]);
					if (yy.addSubgraphMapping) yy.addSubgraphMapping($$[$0 - 7], $$[$0 - 5], _$[$0 - 9], _$[$0]);
					break;
				case 40:
					this.$ = yy.addSubGraph($$[$0 - 3], $$[$0 - 1], {
						text: "",
						type: "text"
					}, "flow");
					if (yy.addSubgraphMapping) yy.addSubgraphMapping($$[$0 - 3], {
						text: "",
						type: "text"
					}, _$[$0 - 5], _$[$0]);
					break;
				case 41:
					this.$ = yy.addSubGraph($$[$0 - 4], $$[$0 - 1], {
						text: "",
						type: "text"
					}, "flow");
					yy.addVertex(this.$, void 0, void 0, void 0, void 0, void 0, void 0, $$[$0 - 3], _$[$0 - 3]);
					if (yy.addSubgraphMapping) yy.addSubgraphMapping($$[$0 - 4], {
						text: "",
						type: "text"
					}, _$[$0 - 6], _$[$0]);
					break;
				case 42:
					...
					if (yy.addSubgraphMapping) yy.addSubgraphMapping(void 0, void 0, _$[$0 - 3], _$[$0]);
					break;
				case 43:
					yy.addGlobal($$[$0 - 1]);
					this.$ = [];
				...
				case 44:
					...
					if (yy.addConnectorMapping) yy.addConnectorMapping($$[$0 - 4], $$[$0 - 2], _$[$0 - 6], _$[$0]);
					break;
				case 45:
					this.$ = yy.addConnector($$[$0 - 5], $$[$0 - 3]);
					yy.addVertex(this.$, void 0, void 0, void 0, void 0, void 0, void 0, $$[$0 - 1], _$[$0 - 1]);
					if (yy.addConnectorMapping) yy.addConnectorMapping($$[$0 - 5], $$[$0 - 3], _$[$0 - 7], _$[$0]);
					break;
				case 46:
					this.$ = yy.addConnector($$[$0 - 1], {
						text: "",
						type: "text"
					});
					if (yy.addConnectorMapping) yy.addConnectorMapping($$[$0 - 1], {
						text: "",
						type: "text"
					}, _$[$0 - 3], _$[$0]);
					break;
				case 47:
					this.$ = yy.addConnector($$[$0 - 2], {
						text: "",
						type: "text"
					});
					yy.addVertex(this.$, void 0, void 0, void 0, void 0, void 0, void 0, $$[$0 - 1], _$[$0 - 1]);
					if (yy.addConnectorMapping) yy.addConnectorMapping($$[$0 - 2], {
						text: "",
						type: "text"
					}, _$[$0 - 4], _$[$0]);
					break;
				...
				case 58:
					...
					if (yy.extendVertexMapping) yy.extendVertexMapping($$[$0 - 1][$$[$0 - 1].length - 1], _$[$0]);
					yy.addLink($$[$0 - 3].stmt, $$[$0 - 1], $$[$0 - 2]);
					if (yy.addEdgeMapping) yy.addEdgeMapping($$[$0 - 3].stmt, $$[$0 - 1], $$[$0 - 2], this._$);
					this.$ = {
						stmt: $$[$0 - 1],
				...
				case 59:
					...
					if (yy.addEdgeMapping) yy.addEdgeMapping($$[$0 - 2].stmt, $$[$0], $$[$0 - 1], this._$);
					this.$ = {
						stmt: $$[$0],
				...
				case 65:
					...
					if (yy.extendVertexMapping) yy.extendVertexMapping($$[$0 - 5][$$[$0 - 5].length - 1], _$[$0 - 4]);
					this.$ = $$[$0 - 5].concat($$[$0]);
					break;
				case 66:
					this.$ = $$[$0 - 4].concat($$[$0]);
				...
				case 69:
					...
					if (yy.addVertexMapping) yy.addVertexMapping($$[$0 - 3], $$[$0 - 1], "square", this._$);
					break;
				case 70:
					this.$ = $$[$0 - 3];
					yy.addVertex($$[$0 - 3], $$[$0 - 1], "doublecircle");
					if (yy.addVertexMapping) yy.addVertexMapping($$[$0 - 3], $$[$0 - 1], "doublecircle", this._$);
					break;
				case 71:
					this.$ = $$[$0 - 5];
					yy.addVertex($$[$0 - 5], $$[$0 - 2], "circle");
					if (yy.addVertexMapping) yy.addVertexMapping($$[$0 - 5], $$[$0 - 2], "circle", this._$);
					break;
				case 72:
					this.$ = $$[$0 - 3];
					yy.addVertex($$[$0 - 3], $$[$0 - 1], "ellipse");
					if (yy.addVertexMapping) yy.addVertexMapping($$[$0 - 3], $$[$0 - 1], "ellipse", this._$);
					break;
				case 73:
					this.$ = $$[$0 - 3];
					yy.addVertex($$[$0 - 3], $$[$0 - 1], "stadium");
					if (yy.addVertexMapping) yy.addVertexMapping($$[$0 - 3], $$[$0 - 1], "stadium", this._$);
					break;
				case 74:
					this.$ = $$[$0 - 3];
					yy.addVertex($$[$0 - 3], $$[$0 - 1], "subroutine");
					if (yy.addVertexMapping) yy.addVertexMapping($$[$0 - 3], $$[$0 - 1], "subroutine", this._$);
					break;
				case 75:
					this.$ = $$[$0 - 7];
					yy.addVertex($$[$0 - 7], $$[$0 - 1], "rect", void 0, void 0, void 0, Object.fromEntries([[$$[$0 - 5], $$[$0 - 3]]]));
					if (yy.addVertexMapping) yy.addVertexMapping($$[$0 - 7], $$[$0 - 1], "rect", this._$);
					break;
				case 76:
					this.$ = $$[$0 - 3];
					yy.addVertex($$[$0 - 3], $$[$0 - 1], "cylinder");
					if (yy.addVertexMapping) yy.addVertexMapping($$[$0 - 3], $$[$0 - 1], "cylinder", this._$);
					break;
				case 77:
					this.$ = $$[$0 - 3];
					yy.addVertex($$[$0 - 3], $$[$0 - 1], "round");
					if (yy.addVertexMapping) yy.addVertexMapping($$[$0 - 3], $$[$0 - 1], "round", this._$);
					break;
				case 78:
					this.$ = $$[$0 - 3];
					yy.addVertex($$[$0 - 3], $$[$0 - 1], "diamond");
					if (yy.addVertexMapping) yy.addVertexMapping($$[$0 - 3], $$[$0 - 1], "diamond", this._$);
					break;
				case 79:
					this.$ = $$[$0 - 5];
					yy.addVertex($$[$0 - 5], $$[$0 - 2], "hexagon");
					if (yy.addVertexMapping) yy.addVertexMapping($$[$0 - 5], $$[$0 - 2], "hexagon", this._$);
					break;
				case 80:
					this.$ = $$[$0 - 3];
					yy.addVertex($$[$0 - 3], $$[$0 - 1], "odd");
					if (yy.addVertexMapping) yy.addVertexMapping($$[$0 - 3], $$[$0 - 1], "odd", this._$);
					break;
				case 81:
					this.$ = $$[$0 - 3];
					yy.addVertex($$[$0 - 3], $$[$0 - 1], "trapezoid");
					if (yy.addVertexMapping) yy.addVertexMapping($$[$0 - 3], $$[$0 - 1], "trapezoid", this._$);
					break;
				case 82:
					this.$ = $$[$0 - 3];
					yy.addVertex($$[$0 - 3], $$[$0 - 1], "inv_trapezoid");
					if (yy.addVertexMapping) yy.addVertexMapping($$[$0 - 3], $$[$0 - 1], "inv_trapezoid", this._$);
					break;
				case 83:
					this.$ = $$[$0 - 3];
					yy.addVertex($$[$0 - 3], $$[$0 - 1], "lean_right");
					if (yy.addVertexMapping) yy.addVertexMapping($$[$0 - 3], $$[$0 - 1], "lean_right", this._$);
					break;
				case 84:
					this.$ = $$[$0 - 3];
					yy.addVertex($$[$0 - 3], $$[$0 - 1], "lean_left");
					if (yy.addVertexMapping) yy.addVertexMapping($$[$0 - 3], $$[$0 - 1], "lean_left", this._$);
					break;
				case 85:
					this.$ = $$[$0];
					yy.addVertex($$[$0]);
					if (yy.addVertexMapping) yy.addVertexMapping($$[$0], void 0, void 0, this._$);
					break;
				case 86:
					$$[$0 - 1].text = $$[$0];
					this.$ = $$[$0 - 1];
		...
		table: [
			...
			{ 83: [1, 230] },
			o($Vv1, [2, 124], { 11: [1, 231] }),
			o($Vv1, [2, 126], { 11: [1, 232] }),
			{ 83: [1, 233] },
			o($Vw1, [2, 199]),
			{
				83: [1, 234],
...
var genColor = /* @__PURE__ */ __name((options) => {
	...
	const border = /* @__PURE__ */ __name((slot) => borderColorArray[slot % borderColorArray.length], "border");
	const fill = /* @__PURE__ */ __name((slot) => hasBkgColors ? `fill: ${bkgColorArray[slot % bkgColorArray.length]};` : "", "fill");
	let sections = "";
...
`;
	...
	for (let i = 0; i < containerSlotCount(paletteLength); i++) {
		...
		const rule = /* @__PURE__ */ __name((suffix) => `${expanded} ${suffix}, ${collapsed} ${suffix}`, "rule");
		sections += `
...
var diagram = {
	...
	styles: /* @__PURE__ */ __name((options) => `${genColor(options)}
  .label {
