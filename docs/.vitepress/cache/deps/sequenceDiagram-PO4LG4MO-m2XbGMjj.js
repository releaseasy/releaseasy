...
import { A as hasKatex, F as renderKatexSanitized, G as setConfig2, H as setAccDescription, O as getUrl, U as setAccTitle, a as clear, b as getConfig, c as configureSvgSize, i as calculateMathMLDimensions, q as setDiagramTitle, r as assignWithDepth_default, s as common_default, v as getAccDescription, w as getDiagramTitle, x as getConfig2, y as getAccTitle, z as sanitizeText } from "./chunk-O7XYJQB3-mKDgYvQm.js";
import { t as require_dist } from "./dist-KRT2YMqB.js";
import { g as utils_default, p as parseFontSize } from "./chunk-ZIGJFQKS-CX9G_8OE.js";
import { n as load, t as JSON_SCHEMA } from "./chunk-LNGE3PJU-CO8MhpBx.js";
import { a as drawRect, c as getTextObj, i as drawImage, n as drawBackgroundRect, r as drawEmbeddedImage, s as getNoteRect } from "./chunk-GWA4HPMP-BXj13iAN.js";
import { t as ImperativeState } from "./chunk-2Q5K7J3B-UBd40NHA.js";
//#region node_modules/.pnpm/mermaid@12.0.0/node_modules/mermaid/dist/chunks/mermaid.core/sequenceDiagram-PO4LG4MO.mjs
...
var actorLabelHeight = /* @__PURE__ */ __name((actor, conf2) => actor.actorTextHeight ?? utils_default.calculateTextDimensions(actor.description ?? "", {
	fontFamily: conf2.actorFontFamily,
	fontSize: conf2.actorFontSize,
	fontWeight: conf2.actorFontWeight
}).height, "actorLabelHeight");
var actorStackHeight = /* @__PURE__ */ __name((textHeight) => GLYPH_BAND_HEIGHT + GLYPH_LABEL_GAP + textHeight + LABEL_LIFELINE_GAP, "actorStackHeight");
var headerBands = /* @__PURE__ */ __name((actorY, rowHeight, textHeight) => {
...
var footerBands = /* @__PURE__ */ __name((actorY, textHeight) => {
...
var neoBands = /* @__PURE__ */ __name((actor, conf2, isFooter, actorY) => {
	if (conf2.look !== "neo") return null;
...
var paletteColor = /* @__PURE__ */ __name((palette, index) => palette?.length ? palette[index % palette.length] : void 0, "paletteColor");
...
var drawActorTypeParticipant = /* @__PURE__ */ __name(function(elem, actor, conf2, isFooter, diagramId, actorIndexMap) {
	...
	if (look === "neo") rectElem.attr("filter", `url(#${dropShadowId(diagramId)})`);
	actor.rectData = rect;
...
var drawActorTypeCollections = /* @__PURE__ */ __name(function(elem, actor, conf2, isFooter, diagramId, actorIndexMap) {
	const actorY = isFooter ? actor.stopy : actor.starty;
	const center = actor.x + actor.width / 2;
	const centerY = actorY + actor.height;
	const { look, theme, themeVariables } = conf2;
	...
	rect.name = actor.name;
	const offset = 6;
	if (conf2.look === "neo") rect.height = actor.height - offset;
	const shadowRect = {
		...rect,
	...
	actor.rectData = rect;
	if (look === "neo") g.attr("filter", `url(#${dropShadowId(diagramId)})`);
	const actorCount = actorIndexMap.get(actor.name) ?? 0;
	if (COLOR_THEMES.has(theme)) {
		rectElem.style("stroke", paletteColor(borderColorArray, actorCount));
		rectElem.style("fill", paletteColor(bkgColorArray, actorCount));
...
var drawActorTypeQueue = /* @__PURE__ */ __name(function(elem, actor, conf2, isFooter, diagramId, actorIndexMap) {
	const actorY = isFooter ? actor.stopy : actor.starty;
	const center = actor.x + actor.width / 2;
	const centerY = actorY + actor.height;
	const { look, theme, themeVariables } = conf2;
  ...
  `;
	...
	actor.rectData = rect;
	if (look === "neo") cylinderGroup.attr("filter", `url(#${dropShadowId(diagramId)})`);
	const actorCount = actorIndexMap.get(actor.name) ?? 0;
	if (COLOR_THEMES.has(theme)) {
...
var drawActorTypeControl = /* @__PURE__ */ __name(function(elem, actor, conf2, isFooter, diagramId, actorIndexMap) {
	...
	if (!bands) actor.height = actElem.node().getBBox().height + 2 * (conf2?.sequence?.labelBoxHeight ?? 0);
	_drawTextCandidateFunc(conf2, hasKatex(actor.description))(actor.description, actElem, rect.x, bands ? bands.labelCenterY - rect.height / 2 : rect.y + r + (!isFooter ? 12 : 5), rect.width, rect.height, { class: `actor ${ACTOR_MAN_FIGURE_CLASS}` }, conf2);
	if (!isFooter) {
...
var drawActorTypeEntity = /* @__PURE__ */ __name(function(elem, actor, conf2, isFooter, diagramId, actorIndexMap) {
	const actorY = isFooter ? actor.stopy : actor.starty;
	const center = actor.x + actor.width / 2;
	const bands = neoBands(actor, conf2, isFooter, actorY);
	const centerY = bands ? bands.lifelineStartY : actorY + 75;
	...
	if (look === "neo") actElem.attr("filter", `url(#${dropShadowId(diagramId)})`);
	const actorCount = actorIndexMap.get(actor.name) ?? 0;
	if (COLOR_THEMES.has(theme)) {
		actElem.style("stroke", paletteColor(borderColorArray, actorCount));
		actElem.style("fill", paletteColor(bkgColorArray, actorCount));
	}
	if (!bands) actor.height = actElem.node().getBBox().height + (conf2?.sequence?.labelBoxHeight ?? 0);
	if (!isFooter) {
		actorCnt++;
		line.append("line").attr("id", "actor" + actorCnt).attr("x1", center).attr("y1", centerY).attr("x2", center).attr("y2", 2e3).attr("class", "actor-line 200").attr("stroke-width", "0.5px").attr("stroke", "#999").attr("name", actor.name).attr("data-et", "life-line").attr("data-id", actor.name);
		actor.actorCnt = actorCnt;
	}
	_drawTextCandidateFunc(conf2, hasKatex(actor.description))(actor.description, actElem, rect.x, bands ? bands.labelCenterY - rect.height / 2 : rect.y + (!isFooter ? 30 : 15), rect.width, rect.height, { class: `actor ${ACTOR_MAN_FIGURE_CLASS}` }, conf2);
	if (!bands) actElem.attr("transform", `translate(0, ${isFooter ? r : r / 2 - 5})`);
	if (!isFooter) {
		actElem.attr("data-et", "participant");
...
var drawActorTypeDatabase = /* @__PURE__ */ __name(function(elem, actor, conf2, isFooter, diagramId, actorIndexMap) {
	const actorY = isFooter ? actor.stopy : actor.starty;
	const center = actor.x + actor.width / 2;
	const bands = neoBands(actor, conf2, isFooter, actorY);
...
`;
	...
	actor.rectData = rect;
	_drawTextCandidateFunc(conf2, hasKatex(actor.description))(actor.description, g, rect.x, bands ? bands.labelCenterY - rect.height / 2 : rect.y + 35, rect.width, rect.height, { class: `actor ${ACTOR_BOX_CLASS}` }, conf2);
	if (!bands) {
		const lastPath = cylinderGroup.select("path:last-child");
...
var drawActorTypeBoundary = /* @__PURE__ */ __name(function(elem, actor, conf2, isFooter, diagramId, actorIndexMap) {
	const actorY = isFooter ? actor.stopy : actor.starty;
	const center = actor.x + actor.width / 2;
	const bands = neoBands(actor, conf2, isFooter, actorY);
	...
	if (!bands) actor.height = actElem.node().getBBox().height + (conf2.sequence.labelBoxHeight ?? 0);
	_drawTextCandidateFunc(conf2, hasKatex(actor.description))(actor.description, actElem, rect.x, bands ? bands.labelCenterY - rect.height / 2 : rect.y + 15, rect.width, rect.height, { class: `actor ${ACTOR_MAN_FIGURE_CLASS}` }, conf2);
	if (!bands) actElem.attr("transform", `translate(0,21)`);
	if (!isFooter) {
		actElem.attr("data-et", "participant");
...
var drawActorTypeActor = /* @__PURE__ */ __name(function(elem, actor, conf2, isFooter, actorIndexMap) {
	...
	const gy = /* @__PURE__ */ __name((offset) => bands ? bands.glyphBottomY - (ACTOR_GLYPH_BOTTOM - offset) * glyphScale : actorY + offset, "gy");
	const gx = /* @__PURE__ */ __name((offset) => center + offset * glyphScale, "gx");
	...
	if (!bands) actor.height = ACTOR_GLYPH_HEIGHT;
	const rect = getNoteRect();
	rect.x = actor.x;
	rect.y = actorY;
	rect.fill = "#eaeaea";
	...
	_drawTextCandidateFunc(conf2, hasKatex(actor.description))(actor.description, actElem, rect.x, bands ? bands.labelCenterY - rect.height / 2 : actorY + 35, rect.width, rect.height, { class: `actor ${ACTOR_MAN_FIGURE_CLASS}` }, conf2);
	return actor.height;
}, "drawActorTypeActor");
var drawActor = /* @__PURE__ */ __name(async function(elem, actor, conf2, isFooter, diagramId, diagObj, actorIndexMap) {
	...
	switch (actor.type) {
		...
		case "participant": return await drawActorTypeParticipant(elem, actor, conf2, isFooter, diagramId, resolvedActorIndexMap);
		case "boundary": return await drawActorTypeBoundary(elem, actor, conf2, isFooter, diagramId, resolvedActorIndexMap);
		case "control": return await drawActorTypeControl(elem, actor, conf2, isFooter, diagramId, resolvedActorIndexMap);
		case "entity": return await drawActorTypeEntity(elem, actor, conf2, isFooter, diagramId, resolvedActorIndexMap);
		case "database": return await drawActorTypeDatabase(elem, actor, conf2, isFooter, diagramId, resolvedActorIndexMap);
		case "collections": return await drawActorTypeCollections(elem, actor, conf2, isFooter, diagramId, resolvedActorIndexMap);
		case "queue": return await drawActorTypeQueue(elem, actor, conf2, isFooter, diagramId, resolvedActorIndexMap);
	}
}, "drawActor");
...
var dropShadowId = /* @__PURE__ */ __name((diagramId) => diagramId ? `${diagramId}-drop-shadow` : "drop-shadow", "dropShadowId");
var insertDropShadow = /* @__PURE__ */ __name(function(elem, conf2, diagramId) {
	const { theme } = conf2;
...
var draw = /* @__PURE__ */ __name(async function(_text, id, _version, diagObj) {
	...
	if (look === "neo") svgDraw_default.insertDropShadow(diagram2, conf, id);
...
async function calculateActorMargins(actors, actorToMessageWidth, boxes) {
	...
	for (const prop of actors.keys()) {
		...
		} else actor.height = actor.wrap ? common_default.getMax(actDims.height, conf.height) : conf.height;
		maxHeight = common_default.getMax(maxHeight, actor.height);
	}
